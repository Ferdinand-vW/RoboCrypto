...
template <PriceSource P> class Prices {

  private:
    std::span<Price<P>> m_prices;

    Prices(time_t interval, std::vector<Price<P>> &prices) : m_interval(interval), m_prices(prices) {}

    Prices(time_t interval, std::span<Price<P>> prices) : m_interval(interval), m_prices(prices) {}
  ...
  public:
    ...
    static common::types::Error<Prices<P>> CreatePrices(time_t interval, std::vector<Price<P>> &prices) {
        ...
        for (int i = 1; i < prices.size(); i++) {
            if (t0 + interval != prices[i].get_time()) {
                return "Non matching time interval for prices: " + std::to_string(t0) + " and " +
                       std::to_string(prices[i].get_time());
            }
    ...
    bool addNewPrice(Price<P> p) {
        if (m_prices.size() <= 0 || m_prices.back().get_time() + m_interval == p.get_time()) {
    ...
    Price<P> operator[](int index) const { return m_prices[index]; }

    std::span<Price<P>> &get_data() { return m_prices; }
    ...
    size_t size() const { return m_prices.size(); }

    auto begin() const { return m_prices.begin(); }
    auto end() const { return m_prices.end(); }
};
} // namespace bypto::data::prices