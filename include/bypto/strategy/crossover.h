...
template <data::price::PriceSource P> class CrossoverData : public csv::CsvConvert<CrossoverData<P>> {
  ...
  public:
    ...
    void put_crossover(time_t time) { m_crossovers.insert(time); }
    ...
    auto csv_data() const {
        ...
        for (int i = 0; i < m_times.size(); i++) {
            auto pp_co = m_crossovers.contains(m_times[i]) ? common::utils::pp_time(m_times[i]) : "";
            res.push_back(std::make_tuple(common::utils::pp_time(m_times[i]), m_prices[i].get_price(), m_fast[i],
                                          m_slow[i], pp_co));
        }
        return res;
    }

    std::array<std::string, 5> csv_header() const { return {"TIME", "PRICE", "SLOW", "FAST", "CROSS OVER"}; }
...
template <typename Ind, data::price::PriceSource P> class Crossover : public Strategy<Crossover<Ind, P>, P> {
  ...
  public:
    ...
    Error<std::optional<Order<OType::Market>>> make_decision(time_t now, long double spendable_qty,
                                                             long double spendable_quote_qty, const Prices<P> &prices) {
    ...
    std::stringstream get_indicator_data() { return m_collector.to_csv(); }
};
