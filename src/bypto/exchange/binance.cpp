...
Binance::Binance(binapi::rest::api &api, int tick_rate_s, int max_run_time_s)
    : m_api(api), m_tick_rate_s(tick_rate_s), m_max_run_time_s(max_run_time_s){};
...
binapi::e_type Binance::convert_order_type(order::OType ot) const {
    ...
    case OType::Limit:
        ...
        break;
    case OType::StopLoss:
        return binapi::e_type::stop_loss;
        break;
    case OType::StopLossLimit:
        return binapi::e_type::stop_loss_limit;
        break;
    case OType::TakeProfit:
        return binapi::e_type::take_profit;
        break;
    case OType::TakeProfitLimit:
        return binapi::e_type::take_profit_limit;
        break;
    case OType::LimitMaker:
        return binapi::e_type::limit_maker;
    }
}
...
binapi::e_time Binance::convert_time_in_force(order::TimeInForce tif) const {
    ...
    case TimeInForce::FOK:
        return binapi::e_time::FOK;
    case TimeInForce::GTC:
        return binapi::e_time::GTC;
    case TimeInForce::IOC:
        return binapi::e_time::IOC;
    }
}
...
void Binance::prepare_tables() {
    ...
    std::vector<std::string> lines = {"CREATE TABLE IF NOT EXISTS prices ("s,
...
void Binance::sync_price_history(time_t oldest) {
    using namespace std::string_literals;
    std::vector<std::string> lines = {"SELECT time,symbol,price", "FROM prices", "WHERE time >= $1;"};
    ...
    for (const auto &row : prices) {
        auto tpl =
            std::make_tuple(Symbol::from_string(row["symbol"].as<std::string>()), row["price"].as<long double>());
...
void Binance::sync_order_history(time_t oldest) {
    ...
    std::vector<std::string> lines = {
        "SELECT time,order_type,symbol,position,quantity,price,base_or_quote,time_in_force,limit,stop", "FROM orders",
        "WHERE time >= $1;"};
    ...
    for (const auto &row : orders) {
        ...
        std::optional<BaseOrQuote> boq = bind(row["base_or_quote"].optional<std::string>(), boq_from_string);
        auto tif = bind(row["time_in_force"].optional<std::string>(), tif_from_string);
        ...
        GenericOrder order(otype_from_string(row["order_type"].as<std::string>()).value(),
                           Symbol::from_string(row["symbol"].as<std::string>()), row["quantity"].as<long double>(),
...
void Binance::sync_account_history(time_t oldest) {
    using namespace std::string_literals;
    std::vector<std::string> lines = {"SELECT time,currency,value", "FROM account", "WHERE time >= $1;"};
...
Error<int> Binance::execute_order(order::GenericOrder go) {
    auto tif = go.m_time_in_force.has_value() ? convert_time_in_force(go.m_time_in_force.value()) : binapi::e_time::FOK;
    ...
    auto resp = m_api.new_order(go.m_sym.to_string().c_str() // symbol
                                ...
                                ,
                                binapi::e_trade_resp_type::FULL, std::to_string(go.m_qty) // quantity
                                ,
...
Error<long double> Binance::get_account_value(time_t t) { auto acc = get_account_info(); }
...
Error<bool> Binance::cancel_order(int o_id) { return std::string("not yet implemented"); }
