...
std::string Symbol::to_string() const { return this->base() + this->quote(); }

bool Symbol::operator==(const Symbol &s1) const { return this->base() == s1.base() && this->quote() == s1.quote(); }

bool Symbol::operator<(const Symbol &rhs) const { return this->to_string() < rhs.to_string(); }
...
Value::Value(std::string currency, long double quantity) : m_currency(currency), m_quantity(quantity){};
...
std::string Value::ppValue() { return std::to_string(m_quantity) + m_currency; }
