...
std::string intercalate(std::vector<std::string> &v) { return intercalate("", v); }

std::string intercalate(std::deque<std::string> &v) { return intercalate("", v); }

std::string intercalate(std::string del, std::vector<std::string> &v) { return intercalate(del, v.begin(), v.end()); }

std::string intercalate(std::string del, std::deque<std::string> &v) { return intercalate(del, v.begin(), v.end()); }
...
time_t add_time(time_t t, time_unit tu) {
    ...
    return create_time(timeinfo->tm_year + 1900, timeinfo->tm_mon + 1,
                       timeinfo->tm_mday + tu.m_days
                       // localtime may be DST, whereas the considered time
                       // will always be in UTC
                       ,
                       timeinfo->tm_hour + tu.m_hours - timeinfo->tm_isdst, timeinfo->tm_min + tu.m_minutes,
...
std::string to_string(const time_unit &tu) {
    std::string res("");
    if (tu.m_days > 0) {
        res += std::to_string(tu.m_days) + "d";
    }
    if (tu.m_hours > 0) {
        res += std::to_string(tu.m_hours) + "h";
    }
    if (tu.m_minutes > 0) {
        res += std::to_string(tu.m_minutes) + "m";
    }
    if (tu.m_seconds > 0) {
        res += std::to_string(tu.m_seconds) + "s";
    }
    return res;
}

bool operator==(const time_unit &tu1, const time_unit &tu2) {
    return tu1.m_seconds == tu2.m_seconds && tu1.m_minutes == tu2.m_minutes && tu1.m_hours == tu2.m_hours &&
           tu1.m_days == tu2.m_days;
}

bool operator!=(const time_unit &tu1, const time_unit &tu2) { return !(tu1 == tu2); }
