...
std::string OutputWriter::filepath(std::string filename) { return m_outdir + "/" + filename + ".csv"; }

OutputWriter::OutputWriter() { prepare_outdir(); }

void OutputWriter::set(OutputType ot) {
    switch (ot) {
    case OutputType::Fund:
        m_write_funds = true;
        break;
    case OutputType::Order:
        m_write_orders = true;
        break;
    case OutputType::PNL:
        m_write_pnls = true;
        break;
    case OutputType::Indicator:
        m_write_indicators = true;
        break;
